...
def plot_describe_profe(df_describe):
...
# Se corrigió el chunk anterior "plot_describe_profe" por: 

def plot_describe(df_describe):
    # Extraer los nombres de las columnas que corresponden a longitudes de onda
    lgtd_onda = [float(i) for i in df_describe.columns.values[5:]]

    # Agrupación de índices para cada subplot
    L = [(1, 2), (3, 7), (4, 5, 6)]
    # Diccionario para mapear el índice con el nombre de la estadística
    d = {
        1: 'mean',
        2: 'std',
        3: 'min',
    ...
    # Estilo del gráfico
    plt.style.use('seaborn-v0_8')  # 'seaborn-v0_8' es más compatible con versiones recientes de matplotlib

    # Crear figura y subplots
    fig, axs = plt.subplots(nrows=3, figsize=(12, 8), constrained_layout=True)

    # Iterar sobre cada subplot
    for a, ax in enumerate(axs):
        for i in L[a]:
            y = df_describe.iloc[i, 5:].astype(float)  # Asegurarse que y es numérico
            ax.plot(lgtd_onda, y, label=d[i])
        ax.set_title(f'Estadísticas: {", ".join(d[i] for i in L[a])}', fontsize=10)
    ...
    # Mostrar la figura
    plt.suptitle('Medidas descriptivas por longitud de onda', fontsize=14)
    plt.show()
